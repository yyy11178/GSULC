...
def train(train_loader, epoch, model, optimizer,warm,correcter=None):
    ...
    for it, (images, labels, ids, path) in enumerate(train_loader):
        ...
        softmax_matrix = F.softmax(logits, dim=1).cpu().detach().numpy()
        ...
        print('Epoch:[{0:03d}/{1:03d}]  Iter:[{2:04d}/{3:04d}]  '
            ...
            (1.0*train_correct_batch.item())/len(prec), loss.item(),
            iter_end_time - iter_start_time,len(prec)))
...
def main():
    ...
    # step 2: train the whole network
    elif step == 2:
        print('===> Step 2 ...')
